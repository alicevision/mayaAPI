...
def animLayer(*args, **kwargs):
    ...
    Flags:
      ...
      - exclusive : exl                (bool)          [create,query,edit]
          Set/get exclusive access for layer                                        Flag can have multiple arguments, passed
          either as a tuple or a list.
...
def copyKey(*args, **kwargs):
    ...
    Flags:
      - animLayer : al                 (unicode)       [create]
          ...
          on this layer, then no keys are copied.                    Flag can have multiple arguments, passed either as a tuple or
          a list.
...
def evaluationManager(*args, **kwargs):
    ...
    available evaluation modes. The special mode 'off' disables the evaluation manager.
    
    Flags:
      - asyncUpdate : asu              (bool)          [create,query]
          This option turns on/off the concurrent update of the Viewport 2.0 database and the evaluation in the Evaluation Graph.
          This can improve performance in those cases when the viewport update takes a long time and is mostly serialized due to
          the structure of the Evaluation Graph. It may help performance in those specific circumstances though you will have to
          measure it on a case-by-case basis.
      ...
      - invalidate : inv               (bool)          [create,query]
          This option invalidates the graph. Value is used to control auto rebuilding on idle (false) or forced (true). This
          command should be used as a last resort.
      ...
      - nodeTypeParallel : ntp         (bool)          [create,query]
          This option is used only when the evaluation manager is in parallelmode but can be set at anytime. It activates or
          deactivates parallel scheduling for the class name argument(s) in the evaluation manager. Legal object values are class
          type names: e.g. transform, skinCluster, mesh. When queried without specified nodes, it returns the list of nodes with
          parallel scheduling active.
    
      - nodeTypeSerializeGlobally : ntg (bool)          [create,query]
          This option is used only when the evaluation manager is in parallelmode but can be set at anytime. It activates or
          deactivates global serialization for the class name argument(s) in the evaluation manager that are not parallelized.
          Legal object values are class type names: e.g. transform, skinCluster, mesh. When queried without specified nodes, it
          returns the list of nodes with global serialization active.
    
      - nodeTypeUntrusted : ntu        (bool)          [create,query]
          This option is used only when the evaluation manager is in parallelmode but can be set at anytime. It activates or
          deactivates untrusted scheduling for the class name argument(s) in the evaluation manager that are not parallelized.
          Such scheduling will allow nodes to be evaluated in critical section, separately of any other evaluation. It should be
          used only as a last resort, since high contention of such evaluation can greatly reduce performance. Legal object values
          are class type names: e.g. transform, skinCluster, mesh. When queried without specified nodes, it returns the list of
          nodes with untrusted scheduling option active.
    
      - safeMode : sfm                 (bool)          [create,query]
          This option activates/deactivates parallel evaluation safe mode. When enabled, parallel execution will fall back to
          serial when evaluation graph is missing dependencies. Detection is happening on scheduling of parallel evaluation, which
          means potential fallback will happen at the next evaluation. WARNING: This mode should be disabled with extreme caution.
          It will prevent parallel mode from falling back to serial mode when an invalid evaluation is detected. Sometimes the
          ...
          evaluation, anything from invalid values, missing evaluation, or even crashes.
      ...
      - upstreamFrom : ust             (unicode)       [create,query]
          ...
          present so any such scripts would have to handle them. Flag can have multiple arguments, passed either as a tuple or a
          list.
...
def shot(*args, **kwargs):
    ...
    Flags:
      ...
      - transitionOutLength : tol      (time)          [create,query,edit]
          Length of the transtion out of the shot.                                          Flag can have multiple arguments,
          passed either as a tuple or a list.
...
def evaluator(*args, **kwargs):
    ...
    Flags:
      ...
      - info : i                       (bool)          [query]
          Gets the evaluator information. Only valid in query mode since the information is generated by the evaluator's internal
          state and cannot be changed. In order to use this flag you must also specify the name in the 'name' argument.
...
def bakeResults(*args, **kwargs):
    ...
    Flags:
      ...
      - smart : sr                     (bool, float)   [create]
          Specify whether to enable smart bake and the optional smart bake tolerance.                       Flag can have multiple
          arguments, passed either as a tuple or a list.
...
def geomBind(*args, **kwargs):
    ...
    Flags:
      ...
      - geodesicVoxelParams : gvp      (int, bool)     [create,query,edit]
          ...
          enabled. Default values are 256 true.                                       Flag can have multiple arguments, passed
          either as a tuple or a list.
...
def keyingGroup(*args, **kwargs):
    ...
    Flags:
      ...
      - minimizeRotation : mr          (bool)          [create,query,edit]
          ...
          keyingGroups. Those will need to set minimizeRotation on their respective keyingGroups                          Flag can
...
def playbackOptions(*args, **kwargs):
    ...
    Flags:
      ...
      - blockingAnim : ba              (bool)          [create,query]
          All tangents playback as stepped so that animation can be viewed in pure pose-to-pose form                        Flag
          can have multiple arguments, passed either as a tuple or a list.
...
def setKeyframe(*args, **kwargs):
    ...
    Flags:
      ...
      - animated : an                  (bool)          [create]
          Add the keyframe only to the attribute(s) that have already a keyframe on. Default: false                         Flag
          can have multiple arguments, passed either as a tuple or a list.
...
def clip(*args, **kwargs):
    ...
    Flags:
      ...
      - rotationOffset : rof           (float, float, float) [create,query]
          Return the channel offsets used to modify the clip's rotation.                                    Flag can have multiple
          arguments, passed either as a tuple or a list.
...
def joint(*args, **kwargs):
    ...
    Flags:
      ...
      - symmetryAxis : sa              (unicode)       [create,edit]
          ...
          only used when the symmetry flag is set to True.                     Flag can have multiple arguments, passed either as
          a tuple or a list.
...
def movieInfo(*args, **kwargs):
    ...
    Flags:
      ...
      - movieTexture : mt              (bool)          [create]
          ...
          movie loaded by that node.                                           Flag can have multiple arguments, passed either as
          a tuple or a list.
...
def sculptTarget(*args, **kwargs):
    ...
    Flags:
      ...
      - regenerate : r                 (bool)          [edit]
          ...
          exist. The name of the new shape is returned.                      Flag can have multiple arguments, passed either as a
          tuple or a list.
...
def blendShape(*args, **kwargs):
    ...
    Flags:
      ...
      - normalizationGroups : ng       (bool)          [query]
          Returns a list of the used normalization group IDs.                       Flag can have multiple arguments, passed
          either as a tuple or a list.
...
def pasteKey(*args, **kwargs):
    ...
    Flags:
      - animLayer : al                 (unicode)       [create]
          ...
          doesn't exist for the specified objects or attributes then the keys won't get pasted.                          Flag can
          have multiple arguments, passed either as a tuple or a list.
...
def animDisplay(*args, **kwargs):
    ...
    Flags:
      ...
      - refAnimCurvesEditable : rae    (bool)          [create,query,edit]
          Specify if animation curves from referenced files are editable.                   Flag can have multiple arguments,
          passed either as a tuple or a list.
...
def clipMatching(*args, **kwargs):
    ...
    Flags:
      ...
      - matchRotation : mr             (int)           [create]
          ...
          rotation 2 - Match projected rotation on ground plane                                    Flag can have multiple
          arguments, passed either as a tuple or a list.
...
def deformerWeights(*args, **kwargs):
    ...
    Flags:
      ...
      - method : m                     (unicode)       [create,query,edit]
          Specify the method used to map the weight during import. Valid values are: index, nearest, and over. The indexmethod
          uses the vertex index to map the weights onto the object. This is most useful when the destination object shares the
          same topology as the exported data. The nearestmethod finds the nearest vertex in the imported data set and sets the
          weight value to that value. This is best used when mapping a higher resolution mesh to a lower resolution. The
          overmethod is similar to the indexmethod but the weights on the destination mesh are not cleared prior to mapping, so
          that unmatched indices keep their weights intact.
